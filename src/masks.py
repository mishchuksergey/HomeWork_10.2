def get_mask_card_number(card_number: str) -> str:
    ...
    if not (len(new_card_number) == 16 and new_card_number.isdigit()):
        raise ValueError ("Ошибка при ввводе номера карты.Введите правильный номер карты:")
    mask = "** ****"
    mask_card_number = new_card_number[:4] + " " + new_card_number[4:6] + mask + new_card_number[12:]
...
def get_mask_account(account: str) -> str:
    ...
    **XXXX"""
    if not (len(account) == 20 and account.isdigit()):
        raise ValueError("Ошибка при вводе номера счета. Введите правильный номер счета.")
    mask = "**"
